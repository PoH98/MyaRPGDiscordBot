...
namespace MyaDiscordBot.ButtonEvent
{
    public class ShopElementHandler : IButtonHandler
    {
        ...
        public Task Handle(SocketMessageComponent message, DiscordSocketClient client)
        {
            ...
            builder.WithButton("武器", "shopType-" + element + "-weapon").WithButton("護甲", "shopType-" + element + "-amor").WithButton("戒指", "shopType-" + element + "-ring").WithButton("頸鏈", "shopType-" + element + "-necklece").WithButton("鞋", "shopType-" + element + "-shoes").WithButton("護盾", "shopType-" + element + "-shield");

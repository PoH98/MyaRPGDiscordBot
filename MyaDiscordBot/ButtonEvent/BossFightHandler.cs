...
namespace MyaDiscordBot.ButtonEvent
{
    internal class BossFightHandler : IButtonHandler
    {
        ...
        {
            return message.StartsWith("boss-");
        }
        ...
        public async Task Handle(SocketMessageComponent message, DiscordSocketClient client)
        {
            ...
            IEnumerable<Models.BossSpawned> bosses = bossService.GetEnemy((message.Channel as SocketGuildChannel).Guild.Id);
            if (bosses.Count() < 1)
            {
                await message.RespondAsync("此Boss已經唔存在！無法展開戰鬥！", ephemeral: true);
                return;
            }
            Models.BossSpawned boss = bosses.FirstOrDefault(x => x.Id.ToString() == message.Data.CustomId.Replace("boss-", ""));
            if (boss == null)
            {
                await message.RespondAsync("此Boss已經唔存在！無法展開戰鬥！", ephemeral: true);
                return;
            }
            Models.BattleResult result = battleService.Battle(boss.Enemy, player);
            bossService.UpdateEnemy((message.Channel as SocketGuildChannel).Guild.Id, boss);
            int coin = result.DamageDealt / 10;
            if (coin < 10)
            {
                coin = 10;
            }
            ...
            if (boss.Enemy.Name == "米講粗口亞")
            {
                ...
                int healAmount = player.HP - player.CurrentHP;
                int wait = healAmount * 2;
                ...
                Models.Resource resource = itemService.GetResource(player);
            ...
            else
            {
                ...
                if (result.IsVictory)
                {
                    Models.Item reward = itemService.GetReward(boss.Enemy, player);
                    bossService.DefeatedEnemy((message.Channel as SocketGuildChannel).Guild.Id, boss);
                    if (reward != null)
                    {
